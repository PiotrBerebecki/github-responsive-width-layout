function bindToggleEvent (toggle, sidebar) {
...
function initPullRequest () {
  ...
  var existingToggle = document.querySelector('.discussion-sidebar-responsive-toggle');
  if (existingToggle && sidebar) return bindToggleEvent(existingToggle, sidebar);
  ...
  bindToggleEvent(toggle, sidebar);
}

chrome.extension.onMessage.addListener(function(request, sender, response) {
  if (request.type === 'PAGE_UPDATE' && request.changeInfo.status === 'complete') {
    initPullRequest();
  }
